...
(function () {
    ...
    Basil.utils = {
        ...
        each: function (obj, fnIterator, context) {
            if (this.isArray(obj)) {
                ...
                for (var key in obj)
                    if (fnIterator.call(context, obj[key], key) === false)
                        return;
            }
        },
    ...
    Basil.Storage = function () {
        var _salt = "b45i1" + (Math.random() + 1).toString(36).substring(7),
            _storages = {},
            ...
            _toStoragesArray = function (storages) {
                if (Basil.utils.isArray(storages)) return storages;
            ...
            _toStoredKey = function (namespace, path, delimiter) {
                ...
                if (_isValidKey(path)) {
                    ...
                    path = Basil.utils.isFunction(path.filter)
                        ? path.filter(_isValidKey)
                        : path;
                    key = path.join(delimiter);
                }
                return key && _isValidKey(namespace)
                    ? namespace + delimiter + key
                    : key;
            },
            _toKeyName = function (namespace, key, delimiter) {
                if (!_isValidKey(namespace)) return key;
        ...
        _storages.cookie = {
            check: function (options) {
                ...
                if (window.self !== window.top) {
                    ...
                    var cookie =
                        "thirdparty.check=" + Math.round(Math.random() * 1000);
            ...
            set: function (key, value, options) {
                ...
                if (options.expireDays) {
                    ...
                    date.setTime(
                        date.getTime() +
                            options.expireDays * 24 * 60 * 60 * 1000
                    );
                ...
                if (options.domain && options.domain !== document.domain) {
                    ...
                    if (
                        document.domain.indexOf(_domain) === -1 ||
                        _domain.split(".").length <= 1
                    )
        ...
        return {
            ...
            setOptions: function (options) {
                this.options = Basil.utils.extend(
                    {},
                    this.options || Basil.options,
                    options
                );
            },
            ...
            set: function (key, value, options) {
                ...
                Basil.utils.tryEach(
                    _toStoragesArray(options.storages),
                    function (storage, index) {
                        if (storage !== where) _storages[storage].remove(key);
                    },
                    null,
                    this
                );
            ...
            get: function (key, options) {
                ...
                Basil.utils.tryEach(
                    _toStoragesArray(options.storages),
                    function (storage, index) {
                        if (value !== null) return false; // break if a value has already been found.
                        value = _storages[storage].get(key, options) || null;
                        value =
                            options.raw === true
                                ? value
                                : _fromStoredValue(value);
                    },
            ...
            keys: function (options) {
                options = options || {};
                var keys = [];
                for (var key in this.keysMap(options)) keys.push(key);
                return keys;
            },
            keysMap: function (options) {
                ...
                Basil.utils.tryEach(
                    ...
                    function (storage) {
                        Basil.utils.each(
                            _storages[storage].keys(
                                options.namespace,
                                options.keyDelimiter
                            ),
                            function (key) {
                                map[key] = Basil.utils.isArray(map[key])
                                    ? map[key]
                                    : [];
